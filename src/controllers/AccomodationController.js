 ...
 */
  static async createHostAccommodation(req, res, next) {
    try {
      ...
      if (imageUrl) {
        ...
        return res.status(201).send({ status: 201, message: 'Accomodation added successfully!', data: createdAcc });
      }
  ...
  static async createRoom(req, res, next) {
    try {
      ...
      if (imageUrl) {
        req.body.images = imageUrl;
        const created = await addRoom(req.body);
        return res.status(201).send({ status: 201, message: 'Room succesifully added', data: created });
      }
      util.setError(415, 'Please Upload a valid image');
        return util.send(res);
    } catch (error) { return next(error); }
